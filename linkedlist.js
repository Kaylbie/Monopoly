...
class Node
{
    constructor(nam, sta, elm, next = null, prev=null)
    {
      this.name=nam;
      this.state=sta;
...
class LinkedList
{
    ...
    //Add new node
    append = function(name, state, element){
      //Create new node
      const node = new Node(name, state, element);
      let current;
    ...
    insert = function(name, state, element, index){
      if(index >= 0 && index <= this.length){
        const node = new Node(name, state, element);
    ...
    //Get the indexOf by name
    indexOf = function(nam){
      let current = this.head,
      index = -1;
